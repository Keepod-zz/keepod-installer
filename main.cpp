...
void ubnUninst()
{
	...
	finmsgb.setText(uninstaller::tr("%1 has been uninstalled.").arg(KEEPOD_INSTALLER_TITLE));
...
int main(int argc, char **argv)
{
	...
	if (!disabledrootcheck)
	{
		...
		if (QString(whoamip.readAll()).remove("\r").remove("\n") != "root")
		{
			...
			rootmsgb.setText(uninstaller::tr("%2 must be run as root. Close it, and re-run using either:<br/><b>sudo %1</b><br/>or:<br/><b>su - -c '%1'</b>").arg(app.applicationFilePath()).arg(KEEPOD_INSTALLER_TITLE));
	...
	if (chkinst.contains("Location"))
	{
		...
		uninstmsgb.setWindowTitle(uninstaller::tr("%1 Uninstaller").arg(KEEPOD_INSTALLER_TITLE));
		uninstmsgb.setText(uninstaller::tr("%1 is currently installed. Remove the existing version?").arg(KEEPOD_INSTALLER_TITLE));
    ...
    uimain.m_szAppNlang = tnapplang;
    uimain.m_szAppDir = QDir::toNativeSeparators(QString("%1/").arg(app.applicationDirPath()));
    uimain.m_szAppLoc = app.applicationFilePath();

#ifndef __APPLE__
    uimain.setWindowIcon(QIcon(":/images/images/mark.png"));
#endif

//	QObject::connect(&app, SIGNAL(lastWindowClosed()), &unetbootin, SLOT(killApplication()));
//    bool automate = uimain.ubninitialize(oppairs);
    uimain.show();
//	if (automate)
