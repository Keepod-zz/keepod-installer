...
bool unetbootin::isarch64 = false;
bool unetbootin::islivecd = false;
bool unetbootin::isnetinstall = false;
bool unetbootin::ishdmedia = false;
bool unetbootin::overwriteall = false;
bool unetbootin::searchsymlinks = false;
bool unetbootin::ignoreoutofspace = false;
bool unetbootin::dontgeneratesyslinuxcfg = false;
bool unetbootin::downloadFailed = false;
bool unetbootin::exitOnCompletion = false;
bool unetbootin::testingDownload = false;
bool unetbootin::issalt = false;
bool unetbootin::redundanttopleveldir = false;
int unetbootin::persistenceSpaceMB = 0;
QString unetbootin::installType = "";
...
QString unetbootin::blkidcommand = "";
QString unetbootin::volidcommand = "";
QString unetbootin::syslinuxcommand = "";
QString unetbootin::extlinuxcommand = "";
QString unetbootin::mke2fscommand = "";
...
/*
    ...
    if (!skipExtraction) {
        QString newPath = targetDrive + "/android/";
        callexternapp("mkdir", "-p "+newPath);
        extractedfiles = extractallfiles(isofile, newPath, listfilesizedirpair.first, filepathnames);
        callexternapp("mkdir", newPath+"/data");
    }
...
QStringList unetbootin::extractallfiles(QString archivefile, QString dirxfilesto, QPair<QStringList, QList<quint64> > filesizelist, QStringList outputfilelist)
{
    ...
    int filestobecopied = filelist.size();
    for (int i = 0; i < filestobecopied; ++i)
	{
        int newprg = PRG_DRIVE_PREPARED+(PRG_ISO_EXTRACTED-PRG_DRIVE_PREPARED)*i/filestobecopied;
...
bool unetbootin::downloadfile(QString fileurl, QString targetfile, int minsize, CUIMain *uimain)
{
	if (fileurl.isEmpty())
	{
        return false;
...
void unetbootin::configsysEdit()
{
    ...
    "menuitem=grub,"KEEPOD_INSTALLER_TITLE"\n"
...
void unetbootin::bootiniEdit()
{
    ...
    QString bootiniText = QString("%1\n%2=\""KEEPOD_INSTALLER_TITLE"\"").arg(bootiniCurText).arg(QDir::toNativeSeparators(QString("%1ubnldr.mbr").arg(targetDrive)));
...
void unetbootin::vistabcdEdit()
{
    ...
    vbcdEditS1 << QString("bcdedit /create /d \""KEEPOD_INSTALLER_TITLE"\" /application bootsector > %1tmpbcdid").arg(targetPath) << endl;
	vbcdEditF1.close();
	if (!warch64)
...
//		targetDev = driveselect->currentText();
        targetDev = taskInfo->disk->m_szDevName;
		devluid = getdevluid(targetDev);
		ginstallDir = "";
		installDir = ginstallDir;
		targetDrive = QString("%1/").arg(locatemountpoint(targetDev));
        if ( targetDrive.compare("NOT MOUNTED") == 0 ) {
            // XXX
            return;
        }
	}
...
void unetbootin::writegrub2cfg()
{
    ...
    "\nmenuentry \""KEEPOD_INSTALLER_TITLE"\" {\n"
	"\tset root=%8\n"
	"\t%1 %2 %3 %4\n"
	"\t%5 %6 %7\n"
	"}\n"
...
//			pdesc1->setText(tr("Installing extlinux to %1").arg(targetDev));
	...
	else
	{
        ...
        emit progress(taskInfo->diskIdOnUI, PRG_BOOTLOADER_INSTALLED);
...
//		this->tprogress->setValue(0);
		...
		{
            int newprg = PRG_BOOTLOADER_INSTALLED+(PRG_FINISHED-PRG_BOOTLOADER_INSTALLED)*i/persistenceSpaceMB;
...
//		this->tprogress->setValue(this->tprogress->maximum());
	...
	if (installType == tr("Hard Disk"))
	{
//        rebootmsgtext->setText(tr("After rebooting, select the "KEEPOD_INSTALLER_TITLE" menu entry to boot.%1\nReboot now?").arg(postinstmsg));
	}
	if (installType == tr("USB Drive"))
	{
#ifndef Q_OS_MAC
...
bool unetbootin::checkGrub1Installed ()
{
    QString cmdres = callexternapp("dpkg", "-l grub");

    if ( cmdres.contains("0.97") ) {
        return true;
    }
